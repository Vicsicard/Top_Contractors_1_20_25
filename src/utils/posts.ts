...
// Cache for total post counts to ensure consistent pagination
let cachedPrimaryTotalCount = 0;
let cachedSecondaryTotalCount = 0;
...
export async function getPosts(page = 1, perPage = 10): Promise<{
  ...
  try {
    ...
    // For the first page or if the cache is older than 5 minutes, we get fresh counts
    let primaryTotalCount = cachedPrimaryTotalCount;
    let secondaryTotalCount = cachedSecondaryTotalCount;
    
    const currentTime = Date.now();
    const cacheExpired = currentTime - lastCountUpdateTime > 5 * 60 * 1000; // 5 minutes
    
    if (page === 1 || cacheExpired || (primaryTotalCount === 0 && secondaryTotalCount === 0)) {
      try {
        ...
        if (!error) {
          ...
          cachedPrimaryTotalCount = primaryTotalCount;
      ...
      try {
        ...
        if (secondaryBlogSupabase) {
          ...
          if (!error) {
            ...
            cachedSecondaryTotalCount = secondaryTotalCount;
      ...
      console.log(`[DEBUG] Using cached counts: Primary: ${primaryTotalCount}, Secondary: ${secondaryTotalCount}`);
    }
